...
const avatarMarkup = run("nexonProfileAvatar({name:'본캐',nexonCharacter:{ocid:'abcdefghijklmnop',image:'https://example.com/avatar.png'}},'summary-avatar')");
...
assert.match(css, /\.nexon-avatar\{[^}]*width:72px;height:72px/);
assert.match(css, /\.nexon-avatar\.settings-avatar\{width:60px;height:60px/);
assert.match(css, /\.nexon-avatar\[hidden\]\{display:none\}/);
assert.match(css, /\.nexon-avatar img\{[^}]*transform:scale\(1\.5\)/);
assert.match(css, /@media\(max-width:360px\)[\s\S]*\.nexon-avatar,.nexon-avatar\.summary-avatar\{width:64px;height:64px\}/);
assert.match(css, /\.nexon-avatar\.settings-avatar\{width:56px;height:56px\}/);
